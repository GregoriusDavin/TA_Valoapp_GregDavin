...
class InitiatorScreen extends StatefulWidget {
  @override
  _InitiatorScreenState createState() => _InitiatorScreenState();
}

class _InitiatorScreenState extends State<InitiatorScreen> {
  late Future<List<Agent>> initiatorAgents;

  @override
  void initState() {
    super.initState();
    initiatorAgents = fetchInitiatorAgents();
  }

  Future<List<Agent>> fetchInitiatorAgents() async {
    final response =
    ...
    if (response.statusCode == 200) {
      ...
      List<Agent> agents = (data['data'] as List)
          ...
          .where((agent) => agent.role.displayName == 'Initiator')
          .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Initiator Agents'),
        backgroundColor: Colors.red,
      ),
      body: FutureBuilder<List<Agent>>(
        future: initiatorAgents,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(
              child: Text('No Initiator agents found.'),
            );
          } else {
