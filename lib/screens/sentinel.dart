...
class SentinelScreen extends StatefulWidget {
  @override
  _SentinelScreenState createState() => _SentinelScreenState();
}

class _SentinelScreenState extends State<SentinelScreen> {
  late Future<List<Agent>> sentinelAgents;

  @override
  void initState() {
    super.initState();
    sentinelAgents = fetchSentinelAgents();
  }

  Future<List<Agent>> fetchSentinelAgents() async {
    final response =
    ...
    if (response.statusCode == 200) {
      ...
      List<Agent> agents = (data['data'] as List)
          ...
          .where((agent) => agent.role.displayName == 'Sentinel')
          .toList();
      ...
      body: FutureBuilder<List<Agent>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(
              child: Text('No Sentinel agents found.'),
            );
          } else {
